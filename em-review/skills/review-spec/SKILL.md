---
name: review-spec
description: 仕様準拠観点のレビュー知識（em-review 動的注入用）。SPEC.md との矛盾・必須動作の欠落・クリティカルロジックの誤実装・データ整合性違反を検出する基準を汎用レビュアーに与えます。SPEC.md 不在時はレビュアーがクリーンにスキップします。オーケストレーター指示以外で自発的にロードするものではありません。
