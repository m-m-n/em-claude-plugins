"""Tests for task0005 (implement-failed-kind): the em-workflow plugin
version bump from 0.1.62 to a value strictly greater, applied to both
registries.

Covers task0005 Acceptance Criteria
(feature-docs/implement-failed-kind/tasks/task0005.md):

- AC-1 (NFR3): the plugin manifest's version and the em-workflow
  marketplace entry's version agree.
- AC-2 (NFR3): that value parses as major.minor.patch, keeps major.minor at
  `0.1`, and has a patch strictly greater than the baseline 62 -- a numeric
  tuple comparison, never a whole-string comparison.
- AC-3 (NFR3): the em-review marketplace entry's version is unchanged at
  its pre-task value.
- AC-4: the marketplace entry is located by its `name` field, not by array
  index, and the lookup has a non-vacuity guard so a missing entry fails
  rather than silently passing.
- AC-5 (NFR4): this module imports only the standard library, and each
  matcher has a negative proof against synthetic data -- one sample left at
  the baseline, one where the two registries disagree -- plus a
  well-formedness guard showing the synthetic sample is itself parseable.

Follows the pattern established by
tests/test_verify_rework_lineage_cap_version_bump.py: the "past baseline"
comparison is a dot-separated numeric tuple, not a whole-string comparison,
so a future two-digit patch component still sorts correctly (a naive string
comparison would get "0.1.9" > "0.1.10" backwards).

Note: `tests/test_llm_led_review_user_docs.py` carries a pre-existing
failure unrelated to this feature -- its `TestEmReviewEntryUnchanged` pins
the em-review marketplace entry as a snapshot with no `version` key, while
that entry already carries one (`0.5.7`) on the tree this task started
from. That failure predates this task, concerns a different feature's test
module, and is out of this task's scope; this module does not touch it or
attempt to fix it.
"""
...
# Pre-task baseline (task0005.md, Design): both registries read 0.1.62
...
EM_REVIEW_VERSION_SNAPSHOT = "0.5.7"
...
class TestValidationDetectsRegressions(unittest.TestCase):
    ...
    FORGED_PRE_BUMP_VERSION = "0.1.62"
    FORGED_VERSION_A = "0.1.63"
    FORGED_VERSION_B = "0.1.64"
    ...
    def test_parse_version_distinguishes_parse_failure_from_none(self):
        ...
        self.assertIsNotNone(_parse_version("0.1.63"))


class TestOwnModuleStdlibOnly(unittest.TestCase):
    """AC-5 (NFR4): this new module imports only the standard library (no
