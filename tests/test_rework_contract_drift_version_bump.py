"""Tests for task0006 (rework-contract-drift): the em-workflow plugin
version bump to a patch strictly greater than 49 in both registries, with no
other field of either manifest changed.

Covers task0006 Acceptance Criteria
(feature-docs/rework-contract-drift/tasks/task0006.md):

- AC-1: `em-workflow/.claude-plugin/plugin.json` parses as JSON and the
  matching entry in `.claude-plugin/marketplace.json` carries exactly the
  same version string.
- AC-2: that version is strictly greater than the pre-change value (49),
  differing only in its patch component.
- AC-3: no other plugin's entry in the marketplace manifest (em-review) is
  modified.
- AC-4: this module fails against the pre-change (`0.1.49`) manifests and
  passes after the bump; the full suite is green under
  `python3 -m unittest discover -s tests` with no third-party import added
  by this module (stdlib-only: json, re, unittest, pathlib -- verified by
  manual review of the import block below, since a module cannot assert its
  own discovery or its own import set from inside itself).
...
established by tests/test_goal_vs_spec_divergence_version_bump.py --
the highest pre-existing BASELINE_PATCH in the tests/*_version_bump.py
family -- raising the baseline patch from 48 to 49 (the value already on
disk when this task started), so this feature's own version-bump module
must go red on the un-bumped `0.1.49` tree, which the predecessor's
baseline of 48 would not do. JSON files are parsed, never pattern-matched.

Per IMPLEMENTATION.md Shared Components ("Plugin version value"), this task
is the single owner of both manifests, so no other task's edit can collide
with this one. Per task0006.md Design, no other field of either manifest
changes -- in particular the plugin/entry descriptions stay as they are,
since other pre-existing modules assert their content. AC-3 is checked via
key-set checks (catches any added/removed field) plus per-field pins for
every field this task must not touch, following the precedent and its
stated rationale in test_goal_vs_spec_divergence_version_bump.py:
descriptions are pinned by a stable substring ANCHOR rather than full
literal text, so a future unrelated feature may legitimately reword a
plugin description while bumping the version without this now-pre-existing
module blocking that edit. The two versions' equality check (AC-1) and the
marketplace top-level/owner/small identity fields (short and structurally
stable across every prior bump in this family) ARE pinned by exact value,
since those are true retention invariants for this task.

Matcher -> negative-proof inventory (Test Notes "Negative proof" + edge
case, mirroring test_goal_vs_spec_divergence_version_bump.py):
...
- Component-wise vs string comparison (AC-2, Test Notes "Compare the
  version components numerically, not as strings") ->
...
# Pre-change baseline: both registries read 0.1.49 before this task's edit.
# The highest pre-existing BASELINE_PATCH in the tests/*_version_bump.py
# family was 48 (test_goal_vs_spec_divergence_version_bump.py).
BASELINE_PATCH = 49
...
def _assert_version_past_baseline(test, version):
    """The ordering matcher (AC-2): a component-wise, numeric semantic-
...
def _assert_versions_equal(test, version_a, version_b):
    """The equality matcher (AC-1): the two registries must report the
...
class TestPluginManifestVersion(unittest.TestCase):
    """AC-2: the plugin manifest's version is past baseline; every other
    field is unchanged."""
...
class TestMarketplaceEntryVersion(unittest.TestCase):
    """AC-1, AC-3: marketplace.json's em-workflow entry version equals the
    plugin manifest's exactly and is past baseline; no other field of the
    file, the em-workflow entry, or the em-review entry changed."""
...
class TestValidationDetectsRegressions(unittest.TestCase):
    ...
    FORGED_VERSION_A = "0.1.50"
    FORGED_VERSION_B = "0.1.51"
    ...
    def test_component_wise_comparison_is_numeric_not_lexicographic(self):
        """AC-2: the comparison is component-wise semantic-version, not a
