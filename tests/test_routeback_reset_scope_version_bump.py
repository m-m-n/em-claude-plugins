"""Tests for task0002 (routeback-reset-scope-consistency): the em-workflow
plugin version bump to a patch strictly greater than 38 in both registries.

Covers task0002 Acceptance Criteria
(feature-docs/routeback-reset-scope-consistency/tasks/task0002.md):

- AC-1 (FR8): `em-workflow/.claude-plugin/plugin.json` parses as JSON, its
  `version` is on the `0.1.x` line with patch strictly greater than 38, and
  its `name` still reads `em-workflow`.
...
- AC-3 (FR8, NFR7): this module exists, is discovered by
  ...
  a durable baseline (patch > 38) rather than a fixed literal.
- AC-4 (NFR7): per IMPLEMENTATION.md D8, each matcher has a negative proof --
  the baseline matcher rejects a forged pre-bump `0.1.38` version string,
...
- AC-5 (NFR8): the full suite passes with every pre-existing module
  unmodified.
...
established by tests/test_recycled_task_id_version_bump.py, raising the
baseline patch from 37 to 38 per IMPLEMENTATION.md D6 (this feature's own
version-bump module must go red on the un-bumped `0.1.38` tree, which the
predecessor's baseline of 37 would not do). JSON files are parsed, never
pattern-matched.
...
class TestMarketplaceEntryVersion(unittest.TestCase):
    """AC-2 (FR8): the em-workflow marketplace entry's version is past
...
class TestValidationDetectsRegressions(unittest.TestCase):
    """AC-4 / NFR7: a negative proof per matcher, plus a non-vacuity guard
    ...
    FORGED_PRE_BUMP_VERSION = "0.1.38"
    FORGED_VERSION_A = "0.1.39"
    FORGED_VERSION_B = "0.1.40"
