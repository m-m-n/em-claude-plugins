...
class TestOnceSectionCoversAllFourPhaseBoundaryKinds(unittest.TestCase):
    ...
    def test_ordinary_step_boundary_names_design_skipped_exception(self):
        # task-tier-reduction/task0005 (FR3): rewritten so the ordinary-step
        # boundary row admits `skipped` regardless of whether design's own
        # skip or a tier's skip caused it, instead of restricting the
        # exception to design alone -- updated in place (not duplicated),
        # same convention as the ITEMS_1_TO_6_VERBATIM update above.
        self.assertIn("`skipped`", self.section)
        self.assertIn("design 自身の skip", self.section)
        self.assertIn("tier による skip", self.section)
