...
(task0003, subagentstop-failed-event).

Covers task0003 Acceptance Criteria
(feature-docs/subagentstop-failed-event/tasks/task0003.md):

- AC-1: the em-workflow plugin manifest version equals the version of the
  marketplace entry whose name is `em-workflow`.
- AC-2: that version parses as three numeric components and is strictly
  greater than the baseline 0.2.3, compared on parsed numeric components,
  never on the raw string.
- AC-4: the marketplace lookup fails loudly when no entry of that name
  exists; each matcher has a negative proof against forged data, plus a
  well-formedness guard on the forged sample; this module imports only the
  standard library.

AC-3 (the value this task writes is 0.2.4 in both files, and the diff of
both files touches only these two version values) and AC-5
(`python3 -m unittest discover -s tests` passes, including the pre-existing
`tests/test_stale_launched_version_bump.py`) are properties of the diff and
of the whole `tests/` run, not of this module alone; they are recorded in
this task's test record rather than re-tested here.
"""
...
def _assert_bump_over_baseline(test, version, baseline):
    ...
    bump -- and never the raw string, which would sort a two-digit patch
    component backwards."""
...
class TestEmWorkflowVersionBump(unittest.TestCase):
    """AC-1, AC-2: the em-workflow manifest and marketplace entry agree and
    are a bump over the recorded pre-feature baseline 0.2.3."""
    ...
    def test_manifest_version_is_a_bump_over_baseline(self):
        _assert_bump_over_baseline(self, self.manifest.get("version"), BASELINE)

    def test_marketplace_entry_version_is_a_bump_over_baseline(self):
        _assert_bump_over_baseline(self, self.entry.get("version"), BASELINE)
...
class TestMarketplaceEntryLookupGuard(unittest.TestCase):
    """AC-4: the lookup fails loudly rather than silently passing when the
...
class TestValidationDetectsRegressions(unittest.TestCase):
    """AC-4: a negative proof per matcher against forged data, plus a
    well-formedness guard on the forged sample."""

    def test_forged_baseline_version_is_well_formed(self):
        """Non-vacuity guard for the bump matcher."""
        baseline_str = ".".join(str(p) for p in BASELINE)
        self.assertIsNotNone(_parse_version(baseline_str))

    def test_bump_matcher_rejects_forged_baseline_version(self):
        baseline_str = ".".join(str(p) for p in BASELINE)
        with self.assertRaises(AssertionError):
            _assert_bump_over_baseline(self, baseline_str, BASELINE)

    def test_matcher_accepts_a_minor_or_major_advance_over_baseline(self):
        # A minor or major bump is a legitimate advance over the baseline --
        # this matcher must not reject it merely because major.minor
        # changed.
        _assert_bump_over_baseline(self, "0.3.0", BASELINE)  # must not raise
        _assert_bump_over_baseline(self, "1.0.0", BASELINE)  # must not raise

    def test_bump_matcher_rejects_malformed_version_shape(self):
        with self.assertRaises(AssertionError):
            _assert_bump_over_baseline(self, "abc", BASELINE)

    def test_equality_matcher_rejects_forged_differing_versions(self):
        with self.assertRaises(AssertionError):
            _assert_versions_equal(self, "0.2.3", "0.2.4")

    def test_forged_differing_versions_are_both_well_formed(self):
        """Non-vacuity guard for the equality matcher."""
        self.assertIsNotNone(_parse_version("0.2.3"))
        self.assertIsNotNone(_parse_version("0.2.4"))
    ...
    def test_matched_name_with_malformed_version_fails_the_parse_assertion(self):
        ...
        with self.assertRaises(AssertionError):
            _assert_bump_over_baseline(self, entry.get("version"), BASELINE)
...
class TestOwnModuleStdlibOnly(unittest.TestCase):
    """AC-4: this new module imports only the standard library (no
