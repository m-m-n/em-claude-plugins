...
- AC-3 (FR3): "its absence only degrades the stop-tool recorder to a
  ...
  and leads to Residual ending in the gate-rejected terminal, names
...
class TestAgentsJsonlParagraphAbsenceBehavior(unittest.TestCase):
    ...
    terminal; orphan recovery stops at `no-agent-entry`; the old
    ...
    def test_no_agent_entry_token_present(self):
        self.assertIn(NO_AGENT_ENTRY_TOKEN, self.paragraph)
