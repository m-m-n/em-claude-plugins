...
- AC-2 (FR13): `.claude-plugin/marketplace.json` parses as JSON; its
  ...
  has `source` `./em-review` and carries a `version` that is a dotted
  numeric string (its identity fields are pinned exactly; the version
  value itself is never compared to a literal, per
...
- `_marketplace_entry`'s lookup (`em-review` source) is a pure regression
  ...
  wording there, so this module exempts it from a negative proof.
