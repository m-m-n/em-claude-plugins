---
name: sdd
description: SDD ワークフロー全体を自動実行する統合エントリポイント。sdd.yaml の進捗を参照し、未完了ステップを順次走らせる。引数で status / update-spec / remove-feature サブコマンドにも切り替え可能
argument-hint: "[path | status [path] | update-spec [path] | remove-feature [path]]"
...
| `remove-feature` | Skill tool で `em-sdd:sdd.remove-feature` を呼ぶ。残りの引数を渡す。完了したら終了 |
...
- **sdd.status / sdd.update-spec / sdd.remove-feature の独立ルート**: `/em-sdd:sdd status` `/em-sdd:sdd update-spec` `/em-sdd:sdd remove-feature` はワークフロー実行ループに入らず、該当 skill を単発で呼ぶだけ。
