...
このスキルは `/em-sdd:sdd` オーケストレータ経由で呼ばれた場合、自動的に次ステップ (`sdd.2-create-plan`) へ続きます。
単独実行で中断した場合は `/em-sdd:sdd` を再実行すれば `sdd.yaml` から続きから再開されます。
```

**注**: 上記出力に「次は /em-sdd:sdd.2-create-plan を実行してください」のような誘導文を含めない。オーケストレータ実行中であってもユーザーへ手動実行を促してしまい、ワークフローが途中で停止する原因になる。
