...
**すべて合格の場合**:
- E2Eテスト結果を確認（Docker環境で自動実行済み）
- 上記の手動テスト項目（E2E不可）を実施
- 手動テスト完了後、VERIFICATION.mdを更新
- 最終コードレビュー、リリース準備へ

**一部要改善の場合**:
- 警告項目を確認して修正を検討（必須ではないが推奨される改善）

**要修正の場合**:
- エラー項目を優先的に修正

**注**: 上記出力に「次のステップ」「次は /em-sdd:sdd.X を実行してください」「`/em-sdd:sdd` を再実行すれば再開できる」のような次工程への誘導文 (案内・自動進行の説明含む) を一切含めない。オーケストレータ実行中にこのテキストがユーザーへ素通しされると、`/sdd` がワークフロー全自動のはずなのに「次は手動で進めてね」と見えてしまう原因になる。完了報告は今この skill が何をやったかだけに留めること。
