...
| TS-5 | Every contributor-tier invocation spelling the harness emits (short flag with separate value, long flag joined by equals, each bare / single-quoted / double-quoted), empty store | Deny for each | Unit |
| TS-6 | Mentions only: an echoed sentence, a text-search pattern argument, a here-document body, a commit-message argument | Exit 0, empty stdout for each | Unit |
| TS-7 | Unrelated commands, a payload whose tool is not the Bash tool, an empty command, unparseable standard input | Exit 0, empty stdout for each | Unit |
...
| TS-18 | Both plugins' `contributor-consent` skill documents: frontmatter and body | Both exist with valid frontmatter; each describes the availability probe and its not-installed exit, the three presented facts (remote visibility, license, sole contributor), exactly one interactive question round, recording through `--record` and revoking through `--remove`; neither presents unpushed-commit state; neither is referenced by a develop-phase document; no corresponding `commands/` file was added | Structural |
...
| TS-22 | A representative sample of invocation and non-invocation payloads, timed | Each run completes well inside the registered 15-second timeout, and the non-invocation short circuit is measurably cheaper than the invocation path | Unit |
...
| SC-8 | Code review is complete | The review phase reports no residual critical or high finding |
...
- [ ] モックとの目視照合: the design step created **no** mockup files and no token
      ...
      the guard's deny reason and additional context (task0001), the skill's
      not-installed line, fact labels and value vocabulary, question wordings and
      option labels, and the no-write line (task0002), and the CLI's four output
      lines (task0001). Wording must match except for the register adjustment
      allowed by IMPLEMENTATION.md Open Questions O1.
- [ ] Run the `contributor-consent` skill once in an interactive session in a real
      ...
      question appears, choosing the consenting option records the repository, and
      re-running the skill offers the maintain / revoke wording.
- [ ] Dismiss the question without answering and confirm nothing is written and the
      no-change line is printed.
...
- Never `ask` (NFR2): no code path emits the `ask` decision, so an unattended run is
  never stalled by a downgraded prompt — TS-21.
...
- Prompt-injection resistance: no user-facing string produced by the guard echoes
  any part of the inspected command, the working directory, the project key or the
  store — TS-19 (constants only, verified by comparing the two copies against the
  fixed text).
