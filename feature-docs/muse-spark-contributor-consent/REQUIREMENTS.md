...
- ユーザーが明示的に同意したリポジトリでのみ contributor ティアを使用可能にし、
  ...
  送ることを無くす。
- そのゲートを、LLM レビュアーやプロンプトインジェクションを受けたエージェントが
  説得によって回避しうる指示文ではなく、コード（PreToolUse(Bash) フック）で決定的に
  強制する。
...
| BO2 | そのゲートを指示文ではなくコード（PreToolUse(Bash) フック）で決定的に強制する。 |
...
| UC03 | `contributor-consent` スキルで同意を記録する | プラグイン利用者 | 高 |
| UC04 | `contributor-consent` スキルで同意を撤回する | プラグイン利用者 | 中 |
...
#### UC03: `contributor-consent` スキルで同意を記録する
...
3. リモートの可視性、ライセンス、単独コントリビューターかどうかの 3 つの事実を機械的に収集し提示する。
4. 単一の `AskUserQuestion` ラウンドで同意を取る。
5. `muse_guard.py --record --project-dir DIR` で同意を記録する。
...
1. 利用者が同じスキルを実行する。
2. スキルが `muse_guard.py --remove --project-dir DIR` を呼ぶ。
...
| FR7 | 同意記録 CLI | `--record` / `--remove` / `--list` を提供する | 高 |
...
| FR13 | 両プラグインへの `contributor-consent` スキル | 同意取得・撤回を手動実行のスキルとして提供する | 高 |
...
GUI は無い。ユーザーに接する面は次の 2 つ。

- `contributor-consent` スキルの対話（3 つの事実の提示と、単一の `AskUserQuestion` ラウンド）。
...
stateDiagram-v2
    ...
    プローブ --> 事実提示: litellm_available == true
    事実提示 --> 同意質問
    同意質問 --> 記録: --record
    同意質問 --> 撤回: --remove
    記録 --> [*]
...
| 課題 | 影響度 | 対応策 |
|------|--------|--------|
| コマンド文字列の静的解析による誤爆（引用符内・grep パターン・ヒアドキュメント本文の言及を起動と誤認） | 高 | 両方向の境界を意識したマッチング（FR5）と、誤爆なし側を網羅するテスト（FR10、TS-3/TS-6/TS-7） |
...
- [ ] AC-12: `skills/contributor-consent/SKILL.md` が両プラグインに存在し、vertex-review/LiteLLM の
      存在プローブとその「未インストール」終了、提示する 3 つの事実、単一の `AskUserQuestion`
      ラウンド、`muse_guard.py --record` による記録、`--remove` による撤回を記述している。
      未 push コミットの状態は提示せず、どの develop フェーズ文書からも参照されていない。
...
- [ ] 境界値: `muse-spark` と `muse-spark-contributor` の superstring 境界（TS-3、TS-4）、
      引数の綴りの全バリエーション（TS-5）、単なる言及と実起動の区別（TS-6）、
      無関係なコマンド・非 Bash ツール・空コマンド・解析不能な stdin（TS-7）。
...
- [x] design ステップの実行（`design-step.recommendation`）: `decide_autonomously`。
      本フィーチャーでは design ステップを実行する（`status: pending`）。ストアの形、コマンド
