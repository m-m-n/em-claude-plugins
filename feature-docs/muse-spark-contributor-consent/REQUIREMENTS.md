...
- ユーザーが明示的に同意したリポジトリでのみ contributor ティアを使用可能にし、
  ...
  送ることを無くす。
- そのゲートを指示文ではなくコードで強制する。強制面は 2 つある。(1) PreToolUse(Bash)
  フックが、同意の無いプロジェクトでの contributor ティア起動を deny する。(2) 同意の
  書き込み CLI（`--record` / `--remove`）が、標準入力が対話端末でない限りストアを
  変更しない。この 2 つが確立する境界は「エージェントの通常の Bash 呼び出し経路からは、
  contributor ティアの起動も同意の記録もできない」という点までであり、あらゆる手段による
  回避が不可能であることまでは主張しない（a12）。
...
| BO2 | そのゲートを指示文ではなくコードで強制する。強制面は 2 つある。(1) PreToolUse(Bash) フックが、同意の無いプロジェクトでの contributor ティア起動を deny する。(2) 同意の書き込み CLI（`--record` / `--remove`）が、標準入力が対話端末でない限りストアを変更しない。この 2 つが確立する境界は「エージェントの通常の Bash 呼び出し経路からは、contributor ティアの起動も同意の記録もできない」という点までであり、あらゆる手段による回避が不可能であることまでは主張しない（a12）。 |
...
| ユーザータイプ | 説明 |
|----------------|------|
| プラグイン利用者（リポジトリ所有者） | `contributor-consent` スキルを対話セッションで手動実行し、提示されたコマンドを自分の端末で実行して当該リポジトリの同意を記録・撤回する。 |
...
3. スキルが `muse_guard.py --list --project-dir DIR` で現在の同意状態を確認する
   （`--list` は FR7 の provenance 制約の対象外）。
4. リモートの可視性、ライセンス、単独コントリビューターかどうかの 3 つの事実を機械的に収集し提示する。
5. 単一の `AskUserQuestion` ラウンドで同意を取る。
6. スキルは `muse_guard.py --record --project-dir DIR` のコマンド行を提示して終了し、
   利用者がそのコマンドを自分の端末で実行する。

**代替フロー**:

- プローブが false の場合、「未インストール」の平文メッセージを出して終了する。
- 利用者がエージェントの Bash ツール経由で同じコマンドを実行した場合、標準入力が
  対話端末でないため書き込みは拒否され、ストアは変更されない（FR7）。

**事後条件**: 利用者が自分の端末でコマンドを実行した後、同意ストアに当該プロジェクトキーと
`updated_at` が記録されている。

#### UC04: `contributor-consent` スキルの提示に従って同意を撤回する
...
1. 利用者が同じスキルを実行する。
2. スキルが `muse_guard.py --remove --project-dir DIR` のコマンド行を提示して終了する。
3. 利用者がそのコマンドを自分の端末で実行する。
...
| FR7 | 同意記録 CLI | `--record` / `--remove` / `--list` を提供し、書き込みは対話端末からのみ受け付ける | 高 |
...
**provenance 制約**: 書き込み経路には provenance 制約を課す。

- `--record` と `--remove` は、標準入力が対話端末（`isatty`）でない限りストアを一切変更せず、
  非ゼロの終了コードと、対話端末から実行する必要があることを述べる短い日本語メッセージを
  stderr に出して終了する。
- この拒否はプロジェクトキー導出やストア読み取りより前に置き、ストアファイルの作成・更新・
  削除・修復を一切伴わない。
- `--list` はこの制約の対象外であり、標準入力が端末であるかによらず従来どおり動作する
  （読み取りのみ、ストアを変更しない）ため、エージェントから呼び出してよい。

**ビジネスルール**:
- ストアを Write / Edit で直接編集することは禁止。すべての変更はこの CLI を通す。
- この CLI はフック経路からもワークフロー自身からも呼ばれない。
- `isatty()` は provenance の代理指標であって、人間が操作した証明ではない（a12）。
...
FR7 の provenance 制約の両側（非対話 stdin での拒否、対話端末での成功）を網羅し、
...
メッセージを出して終了する。そうでなければ、現在の同意状態を `--list` で確認し
（FR7 の provenance 制約の対象外なのでスキル自身が実行してよい）、リモートの可視性
（`gh repo view --json visibility` または同等の手段）、ライセンス、ユーザーが唯一の
コントリビューターかどうか、のちょうど 3 つの事実を機械的に収集して提示し、単一の
`AskUserQuestion` ラウンドで同意を取る。

スキル自身は `--record` / `--remove` を実行しない。回答に応じて、ユーザーが自分の端末で
実行するコマンド行（`python3 <plugin>/hooks/muse_guard.py --record --project-dir <dir>` /
同 `--remove`）をそのまま提示し、実行はユーザーに委ねて終了する。FR7 の provenance 制約に
より、エージェントの Bash ツールから実行しても書き込みは拒否されるためである。
...
- 強制の境界（BO2、FR4、FR7、a12）: 機械的な強制面はフックの deny と CLI の対話端末要求の
  2 つであり、これらが確立するのは「エージェントの通常の Bash 呼び出し経路からは、
  contributor ティアの起動も同意の記録もできない」という境界までである。`isatty()` は
  provenance の代理指標であって人間が操作した証明ではなく、`script` や Python の `pty` で
  擬似端末を割り当てるか、別のプログラムを介すればストアは書き換えうる。学習提供は
  取り消せないため、この残余は仕様に明記したうえで受け入れる。
...
- `contributor-consent` スキルの対話（3 つの事実の提示、単一の `AskUserQuestion` ラウンド、
  ユーザーが自分の端末で実行するコマンド行の提示）。
- deny 時の `permissionDecisionReason`（日本語、同意の欠如を示す）と `additionalContext`
  （非 contributor エントリの使用を指示）。
- `--record` / `--remove` を非対話 stdin で実行したときの stderr メッセージ（日本語、
  対話端末から実行する必要があることを述べる）。
...
- `--record` / `--remove` は標準入力が対話端末でなければストアを変更しない（FR7）。
...
| `isatty()` が provenance の代理指標にとどまり、擬似端末や別プログラム経由の書き込みを防げない | 中 | 保証する境界（エージェントの通常の Bash 呼び出し経路からは記録できない）を仕様に明記し、残余を受け入れる（a12、BO2） |
...
| エージェントがユーザーの意図なく同意を記録する | 中 | 高 | 書き込み経路の対話端末要求（FR7）。ただし保証は通常の Bash 呼び出し経路までであり、擬似端末経由の残余は受け入れる（a12） |
...
- [ ] AC-13: `muse_guard.py --record` および `--remove` は、標準入力が対話端末でないとき、
      ...
      述べる。標準入力が対話端末のとき（擬似端末を割り当てた場合を含む）は従来どおり記録・削除する。
      `--list` は標準入力が端末でなくても動作し、ストアを変更しない。両プラグインのスクリプトの
      コピーで成立する。
...
- [ ] 正常系: 同意記録後の contributor ティア起動が判定なしで通る（TS-2）。同意 CLI の
      ...
      いずれも `--record` / `--remove` は `pty` で割り当てた擬似端末 stdin から実行する。
- [ ] 異常系: 同意ストアが不在・不正 JSON・構造想定外・ディレクトリのとき、contributor ティア
      起動は deny され、素の起動は判定なしで、ストアは書き換えられない（TS-1、TS-8）。
      非対話 stdin での `--record` / `--remove` は非ゼロ終了しストアを変更しない（TS-29）。
...
- [ ] セキュリティ: 未同意プロジェクトからの起動が確実に deny される（TS-1、TS-4）。
      書き込み経路の provenance 境界が両側で検証される（TS-29、TS-30）。
...
| provenance 制約 | `--record` / `--remove` が標準入力の対話端末を要求する制約。`isatty()` は人間が操作した証明ではなく代理指標である（a12） |
...
- [x] design ステップの実行（`design-step.recommendation`）: `decide_autonomously`。
      ...
      変更であるため。（a11）
- [x] 同意書き込みの provenance（`rework.muse-guard-cli-consent-provenance`）: `--record` /
      `--remove` は標準入力が対話端末でなければストアを変更しない。`isatty()` は provenance の
      代理指標であって人間が操作した証明ではなく、擬似端末の割り当てや別プログラム経由の
      書き込みは依然として可能。保証する境界はエージェントの通常の Bash 呼び出し経路までとし、
      残余は仕様に明記して受け入れる。（a12）
