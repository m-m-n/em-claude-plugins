...
| TS20 | `step` value domain in the contract SSOT: the `## Field values` `step` bullet declares the closed domain — the seven `workflow.yaml` step ids (`create-spec`, `design`, `create-plan`, `implement`, `review`, `verify`, `retrospect`) plus the sentinel — extracted structurally, wrap-independently, and compared as a set against the module's own pinned set; the declaration precedes the `no-step` bullet's anchor phrase, so the pre-existing stop-point extraction still matches and still yields exactly `stop-condition-6` / `step-a-abort` / `step-c-abort`, with `create-spec` and `create-plan` absent from it; the executed-step rule, the two precedence rules and the Step C asymmetry are all retained | all assertions pass; a forged bullet lacking the domain declaration is rejected, with a non-vacuity guard | Unit |
| TS21 | Terminal-line emission occasion in `batch-mode.md`: `## Terminal line` states the output condition as the turn having reached one of the terminal states the contract SSOT defines, with normal completion, every terminating stop and the `--once` phase boundary given as instances of that condition rather than limits on it; the run-ending-only phrasing is gone; the contract-document name, the Read-before-emitting instruction and the `` a `--once` phase boundary `` occasion string are retained; the section reports zero contract-literal violations | all assertions pass; a forged section carrying the old run-ending-only definition is rejected, with a non-vacuity guard | Unit |
...
| SC2 | All test scenarios pass | TS1 through TS21 all green |
...
| SC4 | Documentation is complete | The three documents state argument handling, all four phase boundaries, the emission occasions and the value domain, each in its owning layer — the value domain by TS3/TS17/TS20 (contract SSOT), the emission occasions by TS18 (SKILL.md) and TS21 (`batch-mode.md`) |
...
| FR6 | task0001 | TS7 |
| FR7 | task0003, task0005, task0007 | TS3, TS10, TS16 |
| FR8 | task0003, task0005, task0007 | TS3, TS17, TS20 |
| FR9 | task0002, task0006, task0008 | TS4, TS8, TS18, TS21 |
...
| NFR1 | task0002, task0003, task0006, task0007, task0008 | TS4, TS9, TS20 |
| NFR2 | task0001, task0002, task0003, task0004, task0005, task0006, task0007, task0008 | TS1 |
| NFR3 | task0001, task0002, task0003, task0004, task0005, task0006, task0007, task0008 | TS1, TS5, TS19 |
| NFR4 | task0001, task0002, task0003, task0005, task0006, task0007, task0008 | TS3, TS7, TS8, TS10, TS16, TS17 |
| NFR5 | task0002, task0003, task0006, task0008 | TS3, TS4, TS18, TS21 |
