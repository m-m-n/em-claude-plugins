...
| TS-13 (SPEC AC-6) | Reader sites of `HOOK_FILENAMES` and `JOURNAL_ONLY_HOOK_FILENAMES` in `tests/test_recycled_task_id_consistency.py` | Each of `HOOK_FILENAMES` and `JOURNAL_ONLY_HOOK_FILENAMES` is retired together with all references (no definition, 0 reader sites) or has 2 or more reader sites; a reader site is a code reference outside the constant's definition line, and comments and docstrings are not counted | Inspection |
...
- [ ] Count the reader sites of `HOOK_FILENAMES` and `JOURNAL_ONLY_HOOK_FILENAMES` in that module and confirm each is retired together with all references (no definition, 0 reader sites) or has 2 or more reader sites, where a reader site is a code reference outside the constant's definition line and comments and docstrings are not counted (TS-13).
