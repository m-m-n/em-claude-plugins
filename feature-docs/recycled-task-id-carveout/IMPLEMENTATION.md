...
| Test-module ownership map | Prevents two tasks from editing one test module | Precondition: each module below has exactly one owning task AT A TIME. Postcondition: a task modifies only the modules it owns. `tests/test_recycled_task_id_consistency.py` → task0001, then task0004 after task0001 merged (D7). `tests/test_queue_hook_status_read_pin.py` (new) → task0002. `tests/test_recycled_task_id_carveout_version_bump.py` (new) → task0003. `test-docs/recycled-task-id-carveout/task0001.tests.yaml` → task0004 (D7), by exception to the rule that a task writes only its own test record. Every other module under `tests/` is read-only for every task in this feature. | task0001, task0002, task0003, task0004 |
...
### D7: Rework ownership handoff for the consistency module and for task0001's test record

The verify-sourced rework task (task0004) completes the matcher and constant discipline of `tests/test_recycled_task_id_consistency.py`, a module D2's ownership map assigned to task0001. The single-writer rule is preserved by SEQUENCE rather than by exclusivity: task0001 is `merged` before task0004 is dispatched, and no other pending task writes that module, so ownership transfers cleanly instead of two writers overlapping.

The same handoff extends, by exception, to `test-docs/recycled-task-id-carveout/task0001.tests.yaml`. A task normally writes only its own `{T}.tests.yaml` from its own worktree; task0001's record asserts a claim about single-reader constants that does not match the measured state, and correcting it is the record half of the same failed item task0004 fixes in code. Rather than leave the feature's evidence trail contradicting its code, task0004 declares that path in its `files` set and owns the AC-6 entry — and only that entry. This exception is bounded to one file and one entry, and it exists because the record's subject and the code fix are the same finding. Affected: task0004 (and, by exclusion, no other task, all of which are merged).
