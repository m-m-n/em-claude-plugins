...
2. **Lexing / segmentation** — the command string is split into segments and
   ...
   Everything downstream sees words, never raw command text.
...
5. **Decision emission** — a single decision (`allow` / `ask` / `deny`) with a
   stable reason id. A check emits at most one decision and stops.
...
| Recursive-delete check (`check_rm`) | Decide allow / ask / deny for one recursive delete | **Pre**: the command's word list. **Post**: exactly one of `allow` (fall-through, no output), `ask` with reason id `rm-unresolvable`, `deny` with reason id `rm-root` or `rm-recursive`. No new reason id is introduced; the existing `rm-root` check keeps reading the raw token so its reason id survives. | task0001 |
