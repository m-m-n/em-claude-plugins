...
Merging this feature to main required integrating it with the already-merged
feature `routeback-gate-postcondition`, which had rewritten the same I.2.c
route-back bullet. Two expectations below changed as a result; see SPEC.md's
section of the same name for the full mapping.

- **TS-4** — the branch anchor is `When the gate does not hold` (not
  `INAPPLICABLE`), and the expectation is that the rejected path *writes* the
  `implement` step's `status` to `failed`, rather than that `implement` stays
  `failed`.
- **TS-10** — the first `commit-docs.sh` precedes the worktree/branch cleanup,
  reversing the order recorded below.

`tests/test_recycled_task_id_consistency.py` carries the merged expectations and
passes.
