...
| TS-11 | Both registries report `0.1.37` for em-workflow, and `implement-phase.md` has no bare `git commit` / `git add -A` line | `plugin.json` `version` is `0.1.37`; the `em-workflow` marketplace entry is `0.1.37`; the `em-review` entry carries no `version`; zero bare git commit/add lines | Unit |
| TS-12 | Change containment: `git diff --name-only` against the implement baseline commit | Every path is inside {`em-workflow/references/implement-phase.md`, `em-workflow/.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `feature-docs/recycled-task-id-consistency/**`, new modules under `tests/`}; no path under `feature-docs/implement-routeback-gate/`, `em-workflow/hooks/`, `em-workflow/scripts/`, `em-workflow/agents/`, `em-workflow/skills/`; no pre-existing `tests/` module modified | Manual (inspection) |
