...
## Merge Note: integration with `routeback-gate-postcondition`

This document records the design as authored during the feature. When the
feature branch was merged to main, its edit to the `### I.2.c: Failed handling`
route-back bullet of `em-workflow/references/implement-phase.md` conflicted with
the already-merged feature `routeback-gate-postcondition`, which had rewritten
the same bullet. The integration adopted that feature's I.2.c design and
re-expressed this feature's intent (FR3–FR5) in its vocabulary. Where the two
differ, the merged implementation is authoritative:

| As written here | As merged |
| --- | --- |
| FR3 / AC-3: the existing gate ("no task has status `merged`") gains a terminal-journal-last-event precondition alongside it | The gate is a conjunction: no task `merged` AND no task `in_progress`. The `in_progress` half is a union of two sources — workflow.yaml reporting `in_progress`, OR Step I.2.b's last-event-per-task rule reporting a task in-flight. The second source is what rejects a non-terminal (`launched`) last event, so FR3's precondition holds as that union member rather than as a separate check |
| FR4 / AC-4: a non-terminal last event makes route-back INAPPLICABLE and `implement` stays `failed` | Gate rejection takes the rejected path, which *writes* the `implement` step's `status` to `failed` and commits exactly that write — the terminal status write plus its own commit are the only side effect. Same guarantee, stated as an explicit write rather than as an absence |
| Architecture / Data Flow: write set → cleanup → commit | write set → **commit → cleanup**. An unexpected non-zero exit at the route-back commit then stops the phase at a point where no worktree or branch has been deleted |
| TS-4 / TS-10 wording and ordering | `tests/test_recycled_task_id_consistency.py` is authoritative; it was updated to the merged wording and ordering during the integration |

FR1 (I.2.b step 3's reconciled-state write-back), FR2, FR5 and FR6 did not
conflict and merged as written here.
