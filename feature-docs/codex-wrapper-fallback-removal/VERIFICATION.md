...
merged. Task-level acceptance criteria live in the four task plans and are
...
by task0004's new module. TS8 and TS12 are ALSO verified here as a union that
no single task worktree can observe (IMPLEMENTATION.md D4, D6).

### Integrated checks that belong only to this phase

1. **Whole-repository phrase sweep**: neither `whether a fallback provider
   answered` nor `The wrapper's reply may come from a fallback provider`
   occurs anywhere in the repository outside `feature-docs/` (the feature
   documents quote them deliberately, and the superseded feature's records are
   preserved history).
2. **Change-set containment**: the integrated change set, compared against the
   ...
   of the four tasks' `files` entries plus
   `feature-docs/codex-wrapper-fallback-removal/**` and
   `test-docs/codex-wrapper-fallback-removal/**`. In particular it contains
   neither `em-review/scripts/run_codex_exec.sh` nor
   `feature-docs/batch-codex-autonomous-decisions/SPEC.md` (AC7, AC11).
...
| AC9 | Neither removed phrase appears in the four documents; the four pinning tests pass against the edited text | TS8, TS10 |
...
| FR7 | task0001, task0002, task0003 | TS8, TS10 |
| FR8 | task0003 | TS12 |
| FR9 | task0004 | TS9, TS10 |
| NFR1 | task0001, task0002 | TS6, TS11 |
| NFR2 | task0001 | TS1, TS2 |
| NFR3 | task0001 | TS4 |
| NFR4 | task0002, task0003 | TS8 |
| NFR5 | task0001, task0002, task0003, task0004 | TS10 |
| NFR6 | task0002 | TS11 |
