...
acceptance criteria live in `feature-docs/heredoc-stdin-guard/tasks/`.
...
TS-13 are added by this document to give every non-functional requirement a
verifiable item; they introduce no new implementation scope.
...
- Expected: exit code 0 from both; the new guard test file is discovered and its
  cases run.
...
| TS-9 | The full test suite and the destructive-guard case harness after the registration change | Both pass; the suite includes the new guard test file and the existing plugin version parity check | Regression |
...
| SC-5 | AC10: both manifests carry the same bumped em-workflow version | TS-9 (parity check) plus reading both files at verify time |
...
- [ ] AC10: read both manifests and confirm they carry the same bumped version.
...
| Test scenarios (TS-1..TS-13) | 13 | 11 | 0 | 2 (TS-8, TS-12 change-set half) |
| Success criteria (SC-1..SC-8) | 8 | 5 | 0 | 3 (SC-3, SC-4, SC-5) |
