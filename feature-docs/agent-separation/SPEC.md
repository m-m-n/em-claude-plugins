...
- **FR31:** Implement the scope verification procedure in the phase protocols: require a clean integration worktree before dispatch and abort with the offending paths when it is not clean (no automatic cleaning), take the pre-dispatch snapshot of HEAD SHA, the untracked list and the `extend_only` key set (the pinned HEAD SHA carries every tracked path's baseline identity via the clean-worktree precondition, so no whole-index blob-ID/mode listing pass and no whole-tree content-hashing pass runs before dispatch), compute the worker change set from the index and working-tree layers only, hashing content and querying blob IDs and modes solely for the paths a post-dispatch comparison enumerates as changed (deletions, mode changes and file/symlink/absent transitions must still be detected), judge permitted scope by splitting changed paths on pre-dispatch existence, remove violations by restoring tracked files from the snapshot blob and trashing untracked violators with `gio` (aborting when `gio` is unavailable), then evaluate HEAD movement and perform the stale path with `reset --hard` and re-dispatch. Record the exclusivity assumption and its scope in the phase protocols and contracts. Details: design-input.md 5.11.3, 8.9.
