...
2. **マージ提案**: AskUserQuestion —
   ...
   `{base_branch}` にマージする？」
   - **マージする**: メイン作業ツリーがクリーンか確認（feature-docs と
     `test/README.md` の untracked は例外扱い: integration 側と `diff -r`
     （test/README.md は `diff`）で同一確認 → 同一なら
     退避してから `git merge em-workflow/{feature}/integration`。マージで
     同内容が戻る）。退避は fail-closed で行う: `realpath` で対象が
     `{project_root}/feature-docs/` 配下または `{project_root}/test/README.md`
     に解決されること（シンボリックリンク
