...
2. workflow.yaml `project.components` の build / test / format コマンドを
   integration worktree で実行。コマンド文字列は
   `${CLAUDE_PLUGIN_ROOT}/references/command-execution-protocol.md` の承認
   ゲートに従う（allowlist 一致は即実行、それ以外は AskUserQuestion）
3. VERIFICATION.md の Test Scenarios / Success Criteria を評価し、E2E
   コマンドがあれば同ゲートで実行。workflow.yaml で `status: excluded` の
...
2. **マージ提案**: AskUserQuestion —
   ...
   - **マージする**: メイン作業ツリーがクリーンか確認（feature-docs の
     ...
     確認に失敗したら退避せず中断する。dirty なら報告して中断
   - **しない**: ブランチを残す旨と手動マージ手順を 1-2 行で案内
3. **worktree 掃除**: `git worktree remove` で integration worktree を削除
   （ブランチ自体は残す）
