...
- `--pr`: 完了処理で PR を作成するフラグ。起動ごとの設定であり、
  ...
  完了方式を「PR を作成」に固定し、質問を出さない（対話 / batch
  を問わず）。未指定時は従来どおり — 対話は三択を出し、batch は「ブランチを
  残す」を自動選択する。`--batch` / `--once` と併用できる。`--once` 併用時、
  Step C に到達せず途中のフェーズ境界で終わる起動では何も起こさない —
  PR を作るのは Step C を実行する起動だけであり、その起動に `--pr` が
  付いている必要がある（起動ごとの設定なので前の起動からは引き継がれない）
...
メンバーへ写像されるかは上記「tier 決定」手順の永続化ステップが引く
写像表を唯一の定義元とし、ここでは繰り返さない。この記録は実測値を
...
**`--pr` との関係**: `--pr` は Step C（完了処理）を実行する起動でのみ効く。
上記いずれかの境界で終わる起動 — Step C に到達しない起動 — では、`--pr` が
付いていても PR は作られず、完了方式の決定自体が起きない。`--pr` は起動
ごとの設定で前の起動からは引き継がれないため、Step C を実行する起動に
`--pr` を渡す。
...
終端状態に達したすべてのターンについて、この最後の assistant メッセージは
構造化結果のみを含み、それ以外の内容を一切含まない。そのターンが負う
報告 — 停止の原因・影響範囲・リカバリ手順、Step C の完了報告、監査項目の
人間可読な説明 — は、いずれもこの最後のメッセージより前のメッセージで
出力する。この規則は cap 到達走行のような特定のラン種別に限らず、終端
状態に達したターン全般に及ぶ。
