...
- `--batch`: 無人実行モード。**最初に**
  ...
  モード判定は常にこのフラグ）
- パス引数（存在するディレクトリ）: feature directory として扱う
...
2. なければ Glob で `feature-docs/*/workflow.yaml` を探す
   ...
   - 0件: 新規 feature。create-spec フェーズから開始（workflow.yaml は
     create-spec が生成する）。完了後に再探索して確定（batch: タスク記述
     引数を create-spec の入力にする。タスク記述も無ければ中断報告）
...
5. fail → verify を `failed` にし、AskUserQuestion で差し戻し先を確認
   ...
   停止 — batch-mode.md「Rework task synthesis」）
...
2. **マージ提案**: AskUserQuestion —
   ...
   `{base_branch}` にマージする？」
   （batch: 提案せず自動で「マージする」を選ぶ。クリーン確認と退避規律は
   ...
   - **マージする**: メイン作業ツリーがクリーンか確認（feature-docs・
     `test/README.md`・`design-system/` の untracked は例外扱い:
     integration 側と `diff -r`（単一ファイルは `diff`）で同一確認 →
     同一なら退避してから `git merge em-workflow/{feature}/integration`。
     マージで同内容が戻る）。退避は fail-closed で行う: `realpath` で対象が
     `{project_root}/feature-docs/` 配下・`{project_root}/test/README.md`・
     `{project_root}/design-system/` 配下のいずれか
     に解決されること（シンボリックリンク
...
3. **worktree / ブランチ掃除**: `git worktree remove` で integration
   ...
   （マージしなかった場合はブランチを残す）
4. 終了報告: `em-workflow 完了: {feature}`（タスク数 / レビュー
