...
(envelope structure) and 2.3 (applicability). This document is the SSOT
for the common worker input/output envelope structure. Its machine
verification lives in `scripts/validate-worker-output.py` (`--kind
worker-result`), backed by fixtures under `references/fixtures/`
(design-input.md 10.5). Each worker's `payload` shape is defined in that
worker's own contract, `references/contracts/*-contract.md`.
...
| `input_revision` | Revision-identity object (rule R1) | Yes |
| `input_revision`.`workflow_blob` | Blob id of `workflow.yaml` at dispatch time; null before it exists | Yes (value nullable) |
| `input_revision`.`input_digest` | `sha256:`-prefixed digest computed per rule R1 | Yes |
...
| `resolved_input_paths` | Orchestrator-resolved dynamic input paths (rule R1); only the categories the worker's own contract requires are non-empty | Yes |
